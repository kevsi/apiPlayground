      ...
      <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="grid gap-5 py-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {/* Content type */}
            <div className="grid gap-2">
              <Label htmlFor="mock-content-type">Content-Type</Label>
          ...
          {/* Response body */}
          <div className="grid gap-2">
            <Label htmlFor="mock-body">Corps de la réponse</Label>
            <Textarea
              id="mock-body"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-end pb-2 gap-2">
              ...
              />
              <Label htmlFor="mock-enabled" className="cursor-pointer">Route activée</Label>
            </div>
          </div>
            ...
            {rateLimitEnabled && (
              <div className="grid grid-cols-2 gap-4">
                ...
                <p className="col-span-2 text-xs text-muted-foreground">
                  Retourne 429 Too Many Requests si le nombre de requêtes dépasse la limite dans la fenêtre de temps.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="grid gap-3 rounded-lg border p-4">
            <div className="flex items-center justify-between">
              <Label className="font-medium">Scénarios / Variantes</Label>
            ...
            {variants.length === 0 && (
              <p className="text-xs text-muted-foreground">
                Aucune variante. La réponse par défaut sera toujours utilisée.
                Ajoutez des variantes pour simuler différents scénarios (ex: succès 80%, erreur 20%).
              </p>
            )}
            {variants.map((v, i) => (
              <div key={v.id} className="grid gap-2 rounded-lg border bg-muted/30 p-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-medium text-muted-foreground">Variante {i + 1}</span>
                  <Button type="button" variant="ghost" size="sm" onClick={() => removeVariant(i)} className="h-6 w-6 p-0 text-muted-foreground hover:text-red-600">
                ...
                <div className="grid grid-cols-4 gap-2">
                  ...
                  <div className="grid gap-1">
                    ...
                    <Input
                      ...
                      placeholder="1"
                      value={v.weight}
                      onChange={(e) => updateVariant(i, "weight", e.target.value.replace(/\D/g, ""))}
                      className="h-7 text-xs"
                    />
                  </div>
                  <div className="grid gap-1">
                    ...
                    <Input
                      ...
                      placeholder="200"
                      value={v.status}
                      onChange={(e) => updateVariant(i, "status", e.target.value.replace(/\D/g, "").slice(0, 3))}
                      className="h-7 text-xs"
                    />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="grid gap-1">
                    ...
                    <select
                      value={v.contentType}
                      onChange={(e) => updateVariant(i, "contentType", e.target.value)}
                  ...
                  <div className="grid gap-1">
                    ...
                    <Input
                      ...
                      placeholder="0"
                      value={v.delay}
                      onChange={(e) => updateVariant(i, "delay", e.target.value.replace(/\D/g, ""))}
                      className="h-7 text-xs"
                    />
                  </div>
                </div>
          ...
          <div className="grid gap-3">
            <div className="flex items-center justify-between">
              <Label>En-têtes de réponse</Label>
            ...
            {customHeaders.length === 0 && (
              <p className="text-xs text-muted-foreground">
                Aucun en-tête personnalisé. L&apos;en-tête <code className="rounded bg-muted px-1">content-type</code> est géré séparément ci-dessus.
              </p>
            )}
          ...
          <div className="grid gap-3 rounded-lg border p-4">
            <div className="flex items-center justify-between">
              <Label className="font-medium">Correspondance par paramètres d&apos;URL</Label>
            ...
            {matchQueryEntries.length === 0 && (
              <p className="text-xs text-muted-foreground">
                Aucune condition. La route correspondra quel que soit les paramètres de requête.
                Utilisez <code className="rounded bg-muted px-1">*</code> comme valeur pour n&apos;importe quelle valeur.
              </p>
            )}
            {matchQueryEntries.map((e, i) => (
              <div key={i} className="flex items-center gap-2">
                <Input
                  ...
                  value={e.key}
                  onChange={(v) => updateMatchQueryEntry(i, "key", v.target.value)}
                  className="flex-1 font-mono text-xs"
                />
                <span className="text-muted-foreground shrink-0">=</span>
                <Input
                  placeholder="Valeur (ou *)"
                  value={e.value}
                  onChange={(v) => updateMatchQueryEntry(i, "value", v.target.value)}
                  className="flex-1 font-mono text-xs"
                />
                <Button type="button" variant="ghost" size="sm" onClick={() => removeMatchQueryEntry(i)} className="h-8 w-8 p-0 shrink-0 text-muted-foreground hover:text-red-600">
                  <X className="size-3.5" />
                </Button>
              </div>
            ))}
          ...
          <div className="grid gap-3 rounded-lg border p-4">
            <div className="flex items-center justify-between">
              <Label className="font-medium">Correspondance par en-têtes</Label>
            ...
            {matchHeaderEntries.length === 0 && (
              <p className="text-xs text-muted-foreground">
                Aucune condition. La route correspondra quel que soit les en-têtes de la requête.
                Utilisez <code className="rounded bg-muted px-1">*</code> comme valeur pour n&apos;importe quelle valeur.
              </p>
            )}
            {matchHeaderEntries.map((e, i) => (
              <div key={i} className="flex items-center gap-2">
                <Input
                  placeholder="Nom de l'en-tête"
                  value={e.key}
                  onChange={(v) => updateMatchHeaderEntry(i, "key", v.target.value)}
                  className="flex-1 font-mono text-xs"
                />
                <span className="text-muted-foreground shrink-0">:</span>
                <Input
                  placeholder="Valeur (ou *)"
                  value={e.value}
                  onChange={(v) => updateMatchHeaderEntry(i, "value", v.target.value)}
                  className="flex-1 font-mono text-xs"
                />
                <Button type="button" variant="ghost" size="sm" onClick={() => removeMatchHeaderEntry(i)} className="h-8 w-8 p-0 shrink-0 text-muted-foreground hover:text-red-600">
                  <X className="size-3.5" />
                </Button>
              </div>
            ))}
