...
describe("chaining", () => {
    describe("applyCaptures", () => {
        it("captures a simple body field", () => {
            const ctx = makeCtx();
            const captures = [{ name: "userId", expr: "body.id" }];
        ...
        it("captures a nested body field", () => {
            ...
            const ctx = makeCtx();
            const captures = [{ name: "token", expr: "body.data.user.token" }];
        ...
        it("captures from array response", () => {
            ...
            const ctx = makeCtx();
            const captures = [{ name: "firstId", expr: "body[0].id" }];
            const result = applyCaptures(captures, makeResult({ body }), ctx);
            expect(result.firstId).toBe("1");
        });
        it("captures a header value", () => {
            const ctx = makeCtx();
            const captures = [{ name: "requestId", expr: "headers.x-request-id" }];
            const result = applyCaptures(captures, makeResult(), ctx);
            expect(result.requestId).toBe("abc123");
        });
        it("captures the status code", () => {
            const ctx = makeCtx();
            const captures = [{ name: "statusCode", expr: "status" }];
        ...
        it("skips captures when expression doesn't resolve", () => {
            const ctx = makeCtx();
            const captures = [{ name: "missing", expr: "body.nonexistent.deep" }];
            const result = applyCaptures(captures, makeResult(), ctx);
