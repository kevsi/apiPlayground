...
describe("assertions", () => {
    ...
    describe("header assertions", () => {
        it("passes on headers.content-type contains 'json'", () => {
            const result = evaluateAssertion({ expr: "headers.content-type contains 'json'" }, makeResult());
            expect(result.passed).toBe(true);
        });
        it("fails on headers.content-type contains 'xml'", () => {
            const result = evaluateAssertion({ expr: "headers.content-type contains 'xml'" }, makeResult());
            expect(result.passed).toBe(false);
        });
    });
    ...
    describe("custom assertion names", () => {
        it("uses the name property when provided", () => {
            const result = evaluateAssertion({ name: "My custom check", expr: "status == 200" }, makeResult());
    ...
    describe("variable interpolation in assertions", () => {
        it("resolves {{var}} in expected values", () => {
            ...
            const result = evaluateAssertion({ expr: "body.user.id == {{expectedId}}" }, makeResult(), vars);
            expect(result.passed).toBe(true);
        });
        it("resolves {{var}} in expression fields", () => {
            const vars = new Map([["field", "body.user.id"]]);
            const result = evaluateAssertion({ expr: "body.user.id == 1" }, makeResult(), vars);
            expect(result.passed).toBe(true);
        });
    });
    describe("edge cases", () => {
        ...
        it("handles non-JSON body gracefully", () => {
            const result = evaluateAssertion({ expr: "body == 'plain text'" }, makeResult({ body: "plain text" }));
            expect(result.passed).toBe(true);
        });
        it("handles empty body", () => {
            const result = evaluateAssertion({ expr: "status == 200" }, makeResult({ body: undefined }));
            expect(result.passed).toBe(true);
        });
    ...
    describe("assertsPassed", () => {
        it("returns true when all pass", () => {
            const results = evaluateAssertions([{ expr: "status == 200" }, { expr: "body.user.id == 1" }], makeResult());
            expect(assertsPassed(results)).toBe(true);
        });
        it("returns false when any fails", () => {
            const results = evaluateAssertions([{ expr: "status == 200" }, { expr: "status == 404" }], makeResult());
