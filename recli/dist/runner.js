...
import { evaluateAssertions, assertsPassed } from "./assertions.js";
...
function buildEnvVarMap(bundle, envName, dotenv, ctxVars) {
    const map = new Map();
    // 1. ctxVars (from previous iterations/captures) — lowest priority for dotenv
    if (ctxVars) {
        for (const [k, v] of ctxVars)
            map.set(k, v);
    }
...
function buildHeaders(request, ctx) {
    const headers = {};
    if (request.headers) {
...
function buildBody(request, ctx) {
    if (getMethodsWithoutBody(request.method) && request.method !== "GRAPHQL")
        return undefined;
    // GraphQL body
    if (request.method === "GRAPHQL" && request.graphql) {
        const gqlBody = { query: interpolate(request.graphql.query, ctx) };
...
function ensureContentType(headers, body, method) {
    if (!body && method !== "GRAPHQL")
        return;
...
async function checkSnapshot(request, result, snapshotDir, update) {
...
export async function executeRequest(request, ctx, timeoutMs, options) {
...
export function flattenRequests(bundle, requestFilter) {
    const requests = [];
    for (const collection of bundle.collections) {
        ...
        for (const request of collection.requests) {
            if (request.skip)
                continue;
            if (requestFilter && request.name.toLowerCase() !== requestFilter.toLowerCase())
                continue;
...
export async function runWorkspace(filePaths, options) {
    const allResults = [];
...
async function runSequential(requests, ctx, options) {
    const results = [];
 ...
 */
async function runParallel(requests, parentCtx, options) {
