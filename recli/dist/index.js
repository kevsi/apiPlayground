...
for (const [flags, desc, defaultVal] of GLOBAL_OPTIONS) {
    if (defaultVal)
        program.option(flags, desc, defaultVal);
    else
...
program
    ...
    try {
        const bundle = JSON.parse(fs.readFileSync(resolvedPath, "utf8"));
...
async function emitResults(results, opts) {
...
function simpleBodyDiff(before, after) {
    if (!before && !after)
        return "";
    if (!before)
        return "(new)";
...
// --- TUI ---
async function interactiveUI(bundle, filePath) {
...
async function runSingleInteractive(rl, request) {
    console.clear();
