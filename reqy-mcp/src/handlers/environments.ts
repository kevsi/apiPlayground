/**
 * Environment-related tool handlers.
 */
...
export function handleResolveVariables(
  ...
  if (!textToResolve || !resolveEnvName) {
    return {
      content: [{ type: "text", text: "Missing required fields: text, env_name" }],
      isError: true,
    };
  }
  ...
  if (!env) {
    return {
      content: [{ type: "text", text: `Environment not found: ${resolveEnvName}` }],
      isError: true,
    };
  }
    ...
    content: [
      {
        type: "text",
        text: JSON.stringify(
          { original: textToResolve, resolved, env_name: resolveEnvName },
          null,
          2,
        ),
      },
...
export function handleGetEnvironmentVariables(
  ...
  if (!env) {
    return {
      content: [{ type: "text", text: `Environment not found: ${envName}` }],
      isError: true,
    };
  }
  return {
    content: [
      {
        type: "text",
        text: JSON.stringify({ name: env.name, variables: env.variables ?? [] }, null, 2),
      },
    ],
  };
}
...
export function handleCreateEnvironment(
  ...
  return {
    content: [
      {
        type: "text",
        text: JSON.stringify({ created: true, environment_id: env.id, name: env.name }, null, 2),
      },
    ],
  };
}
...
export function handleUpdateEnvironment(
  ...
  return {
    content: [{ type: "text", text: JSON.stringify({ updated: true, environment_id: envId }) }],
  };
}
...
export function handleDeleteEnvironment(
  ...
  return {
    content: [{ type: "text", text: JSON.stringify({ deleted: true, environment_id: envId }) }],
  };
}
...
export function handleDuplicateEnvironment(
  ...
  return {
    content: [
      {
        type: "text",
        text: JSON.stringify({ duplicated: true, new_environment_id: newEnv?.id }, null, 2),
      },
    ],
  };
}
