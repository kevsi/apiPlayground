/**
 * Collection-related tool handlers.
 *
 * Each handler receives the store, validated args, and options,
    ...
    content: [
      {
        type: "text",
        text: JSON.stringify(
          { created: true, collection_id: collection.id, name: collection.name },
          null,
          2,
        ),
      },
...
export function handleUpdateCollection(
  ...
  return {
    content: [{ type: "text", text: JSON.stringify({ updated: true, collection_id: colId }) }],
  };
}
...
export function handleDeleteCollection(
  ...
  return {
    content: [{ type: "text", text: JSON.stringify({ deleted: true, collection_id: colId }) }],
  };
}
    ...
    content: [
      {
        type: "text",
        text: JSON.stringify(
          { duplicated: true, new_collection_id: newCol?.id, name: newCol?.name },
          null,
          2,
        ),
      },
