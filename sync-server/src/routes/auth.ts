...
auth.post("/signup", async (c) => {
  ...
  const token = issueSession(c, { id, email, name: parsed.data.name ?? null });
  return c.json({ user: publicUser({ id, email, name: parsed.data.name ?? null }), token });
});
...
auth.post("/login", async (c) => {
  ...
  const user = db
    ...
    .get(email) as
    { id: string; email: string; name: string | null; password_hash: string | null } | undefined;
