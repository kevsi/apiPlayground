...
describe("routes/auth — signup", () => {
  it("creates a user and returns a session token + user (no password hash leaked)", async () => {
    const app = buildApp();
    ...
    const session = parseSessionCookie(body.token);
...
describe("routes/auth — login", () => {
  it("returns a token for valid credentials", async () => {
    const app = buildApp();
    await app.request("/api/auth/signup", SIGNUP("carol@example.com", "supersecret", "Carol"));
  ...
  it("rejects a wrong password with a generic 401", async () => {
    ...
    const res = await app.request("/api/auth/login", SIGNUP("dave@example.com", "wrongpass"));
...
describe("routes/auth — me / session", () => {
  it("GET /me returns the user when authenticated via Bearer token", async () => {
    ...
    const signup = await app.request(
      "/api/auth/signup",
      SIGNUP("erin@example.com", "supersecret", "Erin"),
    );
  ...
  it("GET /me returns the user when authenticated via session cookie", async () => {
    ...
    const signup = await app.request(
      "/api/auth/signup",
      SIGNUP("frank@example.com", "supersecret"),
    );
  ...
  it("POST /logout clears the session cookie", async () => {
    ...
    const signup = await app.request(
      "/api/auth/signup",
      SIGNUP("grace@example.com", "supersecret"),
    );
    const setCookie = signup.headers.get("set-cookie") ?? "";
