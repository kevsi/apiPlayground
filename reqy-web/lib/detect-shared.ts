...
function getObjectLiteralPropertyString(
...
const HTTP_METHODS_UPPER_ALL = new Set([
  "GET",
  "POST",
  "PUT",
  "DELETE",
...
export function detectFlask(content: string): DetectedRoute[] {
  ...
  for (const m of content.matchAll(ROUTE_RE)) {
    ...
    const hasAuthDec =
      /@login_required|@jwt_required|@token_required|@requires_auth|@permission_required|@fresh_jwt_required/.test(
        head,
      );
