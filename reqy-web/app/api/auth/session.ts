  ...
  if (!_secret) {
    ...
    if (!s) {
      if (typeof process !== 'undefined' && process.env && process.env.NODE_ENV !== 'production') {
        // In development, generate a temp secret so the app doesn't crash
        _secret = "dev-secret-not-for-production-" + Date.now()
        console.warn("[auth] AUTH_SIGNING_SECRET not set — using ephemeral dev secret. Sessions will not persist across restarts.")
        return _secret
      }
      throw new Error("AUTH_SIGNING_SECRET environment variable is not set")
    }
    _secret = s
  }
  return _secret
