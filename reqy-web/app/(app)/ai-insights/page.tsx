...
export default function AiInsightsPage() {
  ...
  const saveConversationHistory = (historyData: ConversationSession[]) => {
    if (typeof window === "undefined") return
    try { void persistence.setItem("ai-conversation-history", JSON.stringify(historyData)) } catch { /* ignore */ }
  }
    ...
    try {
      ...
      try {
        ...
        fireSystemNotification({
          ...
          body: assistantContent.length > 120 ? `${assistantContent.slice(0, 117)}...` : assistantContent,
          event: "aiResponse",
          tag: "ai-response",
        })
      } catch {
      ...
      toast({ title: `Erreur IA : ${String(error)}`, variant: "destructive", meta: { event: "aiError" } })
      fireSystemNotification({
        title: "Erreur IA",
                ...
                {systemNotificationPermission === "denied" && (
                  <div className="p-3 border-b border-border">
                    <p className="text-xs text-muted-foreground">
                      Notifications bloquées par le navigateur. Réactivez-les via l'icône cadenas à gauche de l'URL (Notifications → Autoriser).
                    </p>
                  </div>
                )}
