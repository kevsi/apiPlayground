...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    if (authError) {
      toast({ title: "Erreur de connexion GitHub", description: authError, variant: "destructive" })
  ...
  const [systemPushEnabled, setSystemPushEnabled] = useState<boolean>(
    (typeof window !== 'undefined' && persistence.getItem<string>("probe_system_push_enabled") === "true")
  )
  ...
  const toggleSystemPushEnabled = useCallback(() => {
    setSystemPushEnabled((prev) => {
      const next = !prev
      try { void persistence.setItem("probe_system_push_enabled", next ? "true" : "false") } catch { /* ignore */ }
      return next
    })
  }, [])
  ...
  const handleTestPush = useCallback(() => {
    try { toast({ title: "Test de notification (toast)", meta: { event: "importExport" } }) } catch { /* ignore */ }
  }, [])
