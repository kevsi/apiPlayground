...
(async () => {
  ...
  try {
    const btn = await page.$('button:has-text("New")');
      ...
      const storeAfterNew = await page.evaluate(async () => {
        const key = 'zendeeps-request-store'
        try { const ls = localStorage.getItem(key); if (ls) return ls } catch (_) {}
        try {
          // try reading idb-keyval default DB/store
          const val = await new Promise((res) => {
            ...
            req.onsuccess = () => {
              try {
                ...
                r.onsuccess = () => res(r.result?.value ?? r.result ?? null)
                r.onerror = () => res(null)
              } catch { res(null) }
            }
