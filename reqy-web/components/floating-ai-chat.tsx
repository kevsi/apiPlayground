            ...
            <div className="flex-1 overflow-y-auto space-y-3 p-4 min-h-0">
              {showConfig ? (
                <div className="flex flex-col items-center justify-center gap-3 py-8 text-center px-4">
                  <div className="flex size-10 items-center justify-center rounded-full bg-amber-500/20 border border-amber-500/30">
                    <Sparkles className="size-4 text-amber-300" />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-white">Configure ton IA</p>
                    <p className="mt-1 text-xs text-white/50 leading-relaxed">
                      Choisis un provider et entre ta clé API pour utiliser l'assistant
                    </p>
                  </div>
                  <div className="w-full max-w-[260px] space-y-2">
                    <select
                      ...
                      className="w-full rounded-lg bg-white/10 border border-white/20 px-3 py-2 text-xs text-white focus:outline-none focus:border-violet-500/60 appearance-none"
                    >
                    ...
                    {configProvider !== "ollama" && (
                      <input
                        ...
                        className="w-full rounded-lg bg-white/10 border border-white/20 px-3 py-2 text-xs text-white placeholder:text-white/30 focus:outline-none focus:border-violet-500/60"
                      />
                    )}
                    <button
                      onClick={handleSaveConfig}
                      disabled={configProvider !== "ollama" && !configApiKey.trim()}
                      className="w-full rounded-lg bg-violet-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-violet-500 disabled:opacity-40"
                    >
                      {configProvider === "ollama" ? "Utiliser Ollama" : "Enregistrer et commencer"}
                    </button>
                  </div>
                  <p className="text-[10px] text-white/30">
                    La clé est stockée localement, jamais envoyée ailleurs
                  </p>
                </div>
              ) : messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center gap-3 py-12 text-center">
                  <div className="flex size-12 items-center justify-center rounded-full bg-violet-600/20 border border-violet-500/30">
