...
export function ImportExportModal({ open, onClose }: ImportExportModalProps) {
  ...
  const handleCurlImport = useCallback(() => {
    ...
    if (!targetCollection) {
      setCurlResult({
        success: false,
        message: "Aucune collection disponible. Créez-en une d'abord.",
      });
      return;
    }
    try {
      ...
      setCurlResult({
        success: true,
        message: `Requête ${parsed.method} ${parsed.url} importée dans "${targetCollection.name}".`,
      });
    } catch {
  ...
  const areRequestsEquivalent = useCallback(
    (
      a: { name: string; url: string; endpoint: string; method: string },
      b: { name: string; url: string; endpoint: string; method: string },
    ) =>
      a.name === b.name && (a.url === b.url || a.endpoint === b.endpoint) && a.method === b.method,
    [],
  );
    ...
    async (bundle: ExportBundle, strategy: ConflictStrategy) => {
      ...
      try {
        const collectionMerge = mergeImport({
          ...
          imported: bundle.collections as unknown as {
            id: string;
            updatedAt?: number;
            name?: string;
          }[],
        ...
        const envMerge = mergeImport({
          ...
          imported: bundle.environments as unknown as {
            id: string;
            updatedAt?: number;
            name?: string;
          }[],
      ...
      try {
        ...
        for (const incoming of bundle.collections) {
          const existing = collections.find(
            (c) => c.name === incoming.name || c.id === incoming.id,
          );
          if (!existing) {
            const {
              id: _id,
            ...
            if (strategy === "keep") {
              const newRequests = (incoming.requests || []).filter(
                (req) =>
                  !existing.requests.some((existingReq) => areRequestsEquivalent(existingReq, req)),
              );
        ...
        for (const incoming of bundle.environments) {
          ...
          const existing = environments.find(
            (e) => e.name === incoming.name || e.id === incoming.id,
          );
          if (!existing) {
            const {
              id: _id,
            ...
            if (strategy === "keep") {
              const newVars = (incoming.variables || []).filter(
                (variable) =>
                  !existing.variables.some((existingVar) => existingVar.key === variable.key),
              );
              if (newVars.length) {
      ...
      <DialogContent className="max-w-lg rounded-3xl border-border bg-card shadow-2xl">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle>Sync d'équipe</DialogTitle>
              <p className="text-xs text-muted-foreground">
                Import / Export de collections et environnements
              </p>
            </div>
          </div>
        </DialogHeader>
            ...
            {!conflicts.length && !importResult && (
              <div className="flex items-center gap-3 rounded-xl bg-muted/50 px-4 py-3">
                ...
                <ToggleGroup
                  ...
                  onValueChange={(value) => value && setConflictStrategy(value as ConflictStrategy)}
                >
                  <ToggleGroupItem value="keep" className="text-xs">
                    Garder l'existant
                  </ToggleGroupItem>
                  <ToggleGroupItem value="overwrite" className="text-xs">
                    Écraser
                  </ToggleGroupItem>
                  <ToggleGroupItem value="rename" className="text-xs">
                    Renommer
                  </ToggleGroupItem>
                </ToggleGroup>
              </div>
              ...
              <ul className="mb-3 flex flex-col gap-1 text-xs">
                {conflicts.map((c, i) => (
                  <li key={i} className="text-warning">
                    ...
                    <span className="text-warning/80">({c.type})</span> existe déjà
                  </li>
                ))}
              </ul>
              <p className="mb-3 text-xs text-warning">
                Choisissez comment résoudre les conflits :
              </p>
              <ToggleGroup
                type="single"
                value={conflictStrategy}
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <p
                  className={cn(
                    ...
                    importResult.errors.length > 0 ? "text-destructive" : "text-success",
                  )}
                >
              ...
              <ul className="flex flex-col gap-1 text-xs">
                <li className="text-muted-foreground">
                  ✦ {importResult.added} élément{importResult.added !== 1 ? "s" : ""} ajouté
                  {importResult.added !== 1 ? "s" : ""}
                </li>
                <li className="text-muted-foreground">
                  ✦ {importResult.updated} élément{importResult.updated !== 1 ? "s" : ""} mis à jour
                </li>
                <li className="text-muted-foreground">
                  ✦ {importResult.skipped} élément{importResult.skipped !== 1 ? "s" : ""} ignoré
